...
def test_remove_between_square_brackets():
    assert "this is a " == app.remove_between_square_brackets("this is a [text]")


def test_remove_url():
    assert "welcome to " == app.remove_url("welcome to https://www.google.com/")


def test_strip_html():
    assert "Hello" == app.strip_html("<div><br>Hello</div>")
...
def test_tokenize():
    ...
    comparison = array == app.tokenize(text)
    assert comparison.all() == True


def test_remove_stopwords():
    assert "fake news" == app.remove_stopwords("is it fake news ?")
...
def test_formate_dataset():
    ...
    assert df_returned.sort_index(inplace=True) == app.formate_dataset(df_test).sort_index(inplace=True)


def test_denoise_text():
    assert "fake news site" == app.denoise_text("<div><br>is there fake news on this site https://www.bfmtv.com?</div>")
